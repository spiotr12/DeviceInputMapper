...
Add new modes by simply adding more to the `"modes"`:
...
> **Warning!**  
> There must be at least one mode that is master (does not have a parent)

> **Warning!**  
> Modes are merged in order within config file. You need to ensure that  
> a) parent is before/above child  
> b) there are no recursive inheritances
...
`keyAutoRepeatClick(string key, int delay)` Simulate autorepeat keyboard press and release with delay time between clicks. Kills previous
autorepeat for given key. Available in: (***action***)

`keyDynamicAutoRepeatClick(string key, int delay)` Simulate autorepeat keyboard press and release with delay time between clicks.
Dynamically change delay time. Updates delay for given key, reusing previous autorepeat. E.g.:
`"var delay = Math.Round(1000 * (1 - Math.Abs(value))); keyDynamicAutoRepeatClick(\"A\", delay);"`. Available in: (***action***)
